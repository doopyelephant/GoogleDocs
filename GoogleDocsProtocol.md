...
Google Docs uses a model called [Operational Transformation(OT)](https://en.wikipedia.org/wiki/Operational_transformation), the idea behind OT is that to have realtime collaboration, it is impractical to sync the entire document every time one person types one letter.
